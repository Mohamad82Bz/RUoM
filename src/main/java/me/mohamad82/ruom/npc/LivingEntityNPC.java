...
public abstract class LivingEntityNPC extends EntityNPC {
    ...
    public void startUsingItem(PlayerNPC.InteractionHand interactionHand) {
        Ruom.run(() -> {
            LivingEntityAccessor.getFieldUseItem().set(entity, interactionHand == PlayerNPC.InteractionHand.MAIN_HAND ? equipments.get(EquipmentSlot.MAINHAND) : equipments.get(EquipmentSlot.OFFHAND));
            LivingEntityAccessor.getMethodSetLivingEntityFlag1().invoke(entity, 1, true);
            LivingEntityAccessor.getMethodSetLivingEntityFlag1().invoke(entity, 2, interactionHand == PlayerNPC.InteractionHand.OFF_HAND);
        });
        sendEntityData();
    }
